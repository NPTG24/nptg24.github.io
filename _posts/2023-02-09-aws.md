...
Finalmente guardamos el token en el navegador de la aplicación web del puerto 5000 encontrado por ```nmap``` en un inicio, recordando que del script ```server.py``` nos indica como debe ir el nombre del parámetro con parte del código como ```res.set_cookie("auth",token)```:
