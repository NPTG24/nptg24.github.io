...
* Primero ingresamos {% raw %}```${7*7}```{% endraw %}, como no se interpretó significa que debemos seguir de la tabla la flecha que dice "no funciona".
...
* La tabla nos indica que debemos ahora probar con el payload {% raw %}```{{7*7}}```{% endraw %}, con el cual obtenemos éxito y se interpreta.
...
* Como funcionó ahora debemos validar {% raw %}```{{7*'7'}}```{% endraw %} donde obtenemos como resultado 49, es decir siguiendo la tabla nos estamos enfrentando ante un motor de plantillas "Twig".
