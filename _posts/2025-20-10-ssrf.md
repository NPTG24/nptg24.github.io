...
Para identificar la vulnerabilidad, primero intenta cargar google.com desde la URL vulnerable, ya sea mediante GET o POST (por ejemplo, poniendo http://www.google.com en el parámetro correspondiente).
...
Luego apunta la URL vulnerable a http://192.168.1.16:4646/ssrf. Después, espera la petición entrante para confirmar que el servidor realiza la llamada.
...
Podemos aprovechar una vulnerabilidad SSRF para realizar un escaneo de puertos y así enumerar los servicios que se están ejecutando en el sistema objetivo. En este caso trabajamos con una petición por POST guardada desde BurpSuite; si el flujo fuera por GET, bastaría con poner la URL completa en el parámetro ```-u``` dentro del archivo reqports.txt.
